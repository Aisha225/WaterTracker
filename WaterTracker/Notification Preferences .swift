...
struct Notification_Preferences_: View {
    ...
    var body: some View {
        ...
        VStack {
                ...
                ForEach(intervals, id: \.0) { interval in
                    Button(action: {
                        ...
                        VStack {
                            ...
                            Text(interval.1)
                                .foregroundColor(self.selectedInterval == interval.0 ? .white : .black1)
                                .font(.system(size: 13))
                        }
            ...
            Button(action: {
                ...
                if let interval = Int(selectedInterval ?? "60") {
                    NotificationManager.instance.scheduleNotification(startHour: startHour, endHour: endHour, interval: interval)
                }
    ...
    struct Notification_Preferences_Previews: PreviewProvider {
        static var previews: some View {
            Notification_Preferences_().environmentObject(User(weight: 70))
        }
    }
